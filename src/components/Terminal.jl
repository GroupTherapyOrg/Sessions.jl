...
function Terminal()
    Div(:id => "terminal-container",
        # Reopen bar (hidden by default, shown when terminal is hidden)
        Div(:id => "btn-reopen-terminal",
            :class => "hidden h-6 bg-gray-800 border-t border-gray-700 flex items-center px-3 cursor-pointer hover:bg-gray-700",
            Span(:class => "text-gray-400 text-sm", "▲ Terminal")),
