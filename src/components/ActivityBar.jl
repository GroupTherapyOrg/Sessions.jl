# ActivityBar.jl — @island: explorer/terminal panel toggle buttons
#
# Pure SHARED-signal island (no kwargs — kwargs would collide with
# shared-signal slots; see CellView.jl header for the full write-up).
# Buttons flip `sidebar_open_signal` / `terminal_open_signal`; the
# panel @islands (SidebarPanel, TerminalPanel) own their own DOM
# visibility based on those same signals. ActivityBar's effects only
# touch its own button data-state attribute + sync localStorage.
# It never reaches into #fpanel or #repl-panel anymore.
...
@island function ActivityBar()
    sidebar_open,  set_sidebar_open  = sidebar_open_signal
    terminal_open, set_terminal_open = terminal_open_signal

    # ── Effect: explorer button data-state + localStorage sync ──
    create_effect(() -> begin
        v = sidebar_open()
        js("""
    ...
    # ── Effect: terminal button data-state + localStorage + xterm refit ──
    create_effect(() -> begin
        v = terminal_open()
        js("""
