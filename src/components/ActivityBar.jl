...
#
# Local signals (initialized from kwargs so SessionsApp can seed them from
# localStorage at pre-hydration time — see the patchActivityBarProps IIFE).
# Click handlers only set signals. Effects own DOM + localStorage writes.
#
# `js("…", $N)` inside an on_click closure is unsupported — WasmTarget
# mis-indexes the handler export and every click routes to a js.* import
# instead of the signal-set. Always put interpolated js() in create_effect.
...
@island function ActivityBar(; initial_sidebar::Int = 0, initial_terminal::Int = 0)
    ...
    create_effect(() -> begin
        ...
        js("""
            ...
            var b=document.querySelectorAll('.ab-btn')[0];
    ...
    create_effect(() -> begin
        ...
        js("""
            ...
            var b=document.querySelectorAll('.ab-btn')[2];
            if(b)b.setAttribute('data-state',\$1?'on':'off');
