...
function ReplPanel()
    Div(:id => "repl",
        ...
        Div(:id => "term-tab-bar",
            ...
            # Toggle terminal orientation (horizontal ↔ vertical)
            Button(:id => "term-orient-btn",
                :class => "shrink-0",
                :style => "background:none;border:none;cursor:pointer;color:var(--text-3);padding:4px 6px;font-size:12px;transition:color .12s;",
                :title => "Toggle terminal orientation",
                :on_click => "window._sessionsToggleTerminalOrientation&&_sessionsToggleTerminalOrientation()",
                RawHtml("""<svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M3 8h10M10 5l3 3-3 3"/></svg>"""))),
