    ...
    Div(:class => "cell-island",
        ...
        Div(:class => "cell-eye",
            ...
            Div(:style => "position:relative;width:14px;height:14px;",
                # Open eye (always in DOM — default visible state)
                RawHtml("""<svg class="eye-open" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>"""),
                # Closed eye (layered on top when code is hidden)
                Show(() -> is_open() == Int32(0)) do
                    Div(:class => "eye-closed-overlay",
