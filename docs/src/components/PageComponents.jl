...
"""Theme toggle button with inline JS (no Wasm/Suite.jl dependency)."""
function ThemeToggle(; kwargs...)
    ...
    Div(:class => "inline-flex items-center", kwargs...,
        ...
        RawHtml("""<script>(function(){var b=document.getElementById('$(id)');if(!b)return;b.addEventListener('click',function(){document.documentElement.classList.toggle('dark');localStorage.setItem('theme',document.documentElement.classList.contains('dark')?'dark':'light')})})();</script>"""))
end
