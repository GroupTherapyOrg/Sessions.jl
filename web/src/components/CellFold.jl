# CellFold.jl — @island: WASM-driven code fold/unfold per cell
#
# Wraps a code-cell div. Signal controls visibility via Show().
# Eye icon uses stacked open/closed SVGs (same pattern as CellToggle).
# Persistence to server handled by JS MutationObserver in Layout.jl.

@island function CellFold(children...; initial_open=1)
    is_open, set_is_open = create_signal(Int32(initial_open))

    Div(:class => "cell-fold-wrap",
        # Eye toggle — stacked icons, click flips signal
        Div(:class => "cell-eye",
            :on_click => () -> begin
                if is_open() == Int32(1)
                    set_is_open(Int32(0))
            ...
            # Stacked eye icons (CellToggle pattern):
            # Closed eye always in DOM, open eye layered on top via Show
        ...
        # Code cell — shown/hidden by signal
        Show(is_open) do
            children
        end)
end
